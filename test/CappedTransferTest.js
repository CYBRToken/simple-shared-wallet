...
contract('CappedTransfer', (accounts) => {
  ...
  it('Should fail checkIfValidTransfer(uint256) when NOT comply with: amount > 0', async () => {
    ...
    let result = await truffleAssert.fails(
      contractCappedTransfer.checkIfValidTransfer(0, {from: accounts[0]}),
      'revert',
    );
  });
  it('Should fail checkIfValidTransfer(uint256) when NOT comply with: amount <= _maximumTransfer', async () => {
    await contractCappedTransfer.setCap(95, 255, {from: accounts[0]});
    let result = await truffleAssert.fails(
      contractCappedTransfer.checkIfValidTransfer(96, {from: accounts[0]}),
      'revert',
    );
  });
  it('Should fail checkIfValidTransfer(uint256) when NOT comply with: amount > 0', async () => {
  ...
  it('Should fail checkIfValidWeiTransfer(uint256) when NOT comply with: amount > 0', async () => {
    ...
    let result = await truffleAssert.fails(
      contractCappedTransfer.checkIfValidWeiTransfer(0, {from: accounts[0]}),
      'revert',
    );
  });
  it('Should fail checkIfValidWeiTransfer(uint256) when NOT comply with: amount <= _maximumTransferWei', async () => {
    await contractCappedTransfer.setCap(0, 1532892062, {from: accounts[0]});
    let result = await truffleAssert.fails(
      contractCappedTransfer.checkIfValidWeiTransfer(1532892063, {
        from: accounts[0],
      }),
      'revert',
    );
  ...
  it('Should fail setCap(uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(
      contractCappedTransfer.setCap(2014223716, 26, {from: accounts[9]}),
      'revert',
    );
  });
  it('Should fail setCap(uint256,uint256) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(
      contractCappedTransfer.setCap(2014223716, 26, {from: accounts[0]}),
      'revert',
    );
  });
});
